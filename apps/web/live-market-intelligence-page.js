...
function renderDataSourcesValidation() {
  ...
  const sourceCards = sources.map(source => `<article class="live-source-card ${source.status.toLowerCase().replaceAll("_","-")}">
    <div><small>${source.required ? "REQUIRED" : "OPTIONAL"} / ${source.envKey}</small>${badge(source.status)}</div>
    ...
    ${source.probeError ? `<p class="live-source-error">${source.probeError}</p>` : ""}
  </article>`).join("");
  ...
  return `<section class="live-page">
    <header class="live-header"><div><small>DATA SOURCES VALIDATION / CARD 1 / LIVE ADAPTERS</small><h1>Data Sources & Feed Health</h1><p>Verify availability, API connectivity, latency, freshness and data quality before workflow admission. This page displays runtime probe evidence only.</p></div><aside>${badge(gate.workflowPermission)}<span>MODE / LIVE ADAPTERS ONLY</span><span>LAST PROBE / ${formatTime(data.probedAt)}</span></aside></header>
    <div class="live-actions"><button data-live-test>Run Live Validation</button><button data-live-refresh>Refresh Dashboard</button><a href="/executive-command-center/workflow-dashboard">Open Card 1 Test Harness</a><a href="/workspace/market-intelligence/institutional-cot">Open Official CFTC COT</a></div>
...
function render(slug) {
  ...
  return `<section class="live-page">
    ...
    <div class="live-actions"><button data-live-refresh>Refresh Live Probe</button><a href="/executive-command-center/workflow-dashboard">Run Card 1 Completion Test</a><a href="/workspace/market-intelligence/institutional-cot">Open Official CFTC COT</a></div>
    ...
    ${source && source.status === "NOT_CONFIGURED" ? `<article class="live-card live-empty"><h2>No live ${title.toLowerCase()} adapter is configured.</h2><p>${source.configuration}</p><p>The previous illustrative dataset has been removed. Configure a supported connector and refresh the probe to populate this page.</p></article>` : ""}
...
export function bindLiveMarketIntelligencePage(slug) {
  document.querySelectorAll("[data-live-refresh]").forEach(button => button.addEventListener("click", () => load(slug)));
  document.querySelectorAll("[data-live-test]").forEach(button => button.addEventListener("click", () => runLiveValidation(slug)));
}
...
async function runLiveValidation(slug) {
  try {
    ...
    data = (await response.json()).event;
    error = "";
  } catch (reason) {
    error = reason.message;
  }
